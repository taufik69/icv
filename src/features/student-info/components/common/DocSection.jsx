  ...
  return (
    <Reveal as="section" id={section.id} aria-labelledby={`${section.id}-title`} className="min-w-0 scroll-mt-28 rounded-3xl bg-surface p-5 break-words shadow-card ring-1 ring-line-soft md:p-10">
      <div className="flex items-start gap-4">
        {number && (
          <span className="grid size-12 shrink-0 place-items-center rounded-2xl bg-secondary font-heading text-lg font-extrabold text-primary">
            {number}
          </span>
        )}
        <h2 id={`${section.id}-title`} className="pt-1.5 text-2xl leading-snug md:text-3xl">
          {section.title}
        </h2>
      </div>
      {section.intro && <p className="mt-5 text-lg leading-relaxed text-ink-muted">{section.intro}</p>}
