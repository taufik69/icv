...
// One step on the vertical timeline: native <details> accordion (keyboard + screen-reader friendly, no JS).
export function RplStep({ step, index, last }) {
  return (
    <li className="relative pl-16 md:pl-20">
      {!last && <span aria-hidden="true" className="absolute top-14 bottom-0 left-6 w-0.5 bg-linear-to-b from-primary to-line md:left-7" />}
      <span className="absolute top-2 left-0 grid size-12 place-items-center rounded-full bg-secondary font-heading text-lg font-extrabold text-primary shadow-brand ring-4 ring-surface md:size-14">
        {index + 1}
      </span>
      ...
      <details open={index === 0} className="group mb-6 rounded-2xl bg-surface-muted/60 ring-1 ring-line-soft open:bg-surface open:shadow-card">
        <summary className="flex cursor-pointer list-none items-center gap-4 p-5 md:p-6 [&::-webkit-details-marker]:hidden">
          <span className="flex-1">
            ...
            <span className="mt-1 block font-heading text-xl font-bold text-secondary">{step.title}</span>
          </span>
        ...
        <div className="space-y-3 px-5 pb-6 leading-relaxed text-ink-muted md:px-6">
