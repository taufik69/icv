...
- Endless vertical scrollers use `styles/marquee.css` (`marquee-up`, `marquee-mask`, `[--marquee-duration:NNs]`, `[--marquee-direction:reverse]`): render items twice, hide the duplicate below `md` / for reduced motion (see `news/NewsMarqueeColumn`). No framer-motion/motion dependency — keep motion in CSS.
